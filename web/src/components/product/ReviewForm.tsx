...
export function ReviewForm({ productId }: { productId: string }) {
  ...
  async function handleFiles(e: ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of files.slice(0, room)) {
      ...
      try {
        const res = await fetch("/api/reviews/photos", { method: "POST", body: formData });
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/reviews", {
        ...
        body: JSON.stringify({ productId, rating, title: title.trim(), body, photos }),
      });
      const data = await res.json();
      ...
      setSaved(true);
      setOwn({ id: data.id, rating, title: title.trim() || null, body, photos, isHidden: false });
      router.refresh();
    } catch {
  ...
  if (account === undefined) {
    return <div className="h-32 rounded-[22px] border border-line bg-paper-2" aria-hidden />;
  }
    ...
    return (
      <div className="rounded-[22px] border border-line bg-paper-2 p-6 text-center sm:p-8">
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm text-ink-soft">
          Reviews are tied to an account so people can tell real ones from noise.
        </p>
        <Link
    ...
    >
      ...
      {own?.isHidden && (
        <p className="mt-3 rounded-lg bg-paper-3 px-3 py-2 text-xs text-ink-soft">
          This review is currently hidden by the shop, so it isn&rsquo;t shown on
          the storefront. Editing it won&rsquo;t restore it.
        </p>
      )}
      ...
      <label className="mt-5 block">
        <span className="text-xs font-medium text-ink-soft">Headline (optional)</span>
        <input
          type="text"
          value={title}
          maxLength={120}
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          {photos.map((photo, i) => (
            ...
            >
              <Image src={photo} alt={`Attached photo ${i + 1}`} fill sizes="64px" className="object-cover" />
              <button
                type="button"
      ...
      {saved && !error && (
        <p className="mt-4 text-sm font-semibold">Thanks — your review is live.</p>
      )}
